...
def detect_matches(file_names, fields=None, tol_field=None, tolerance=0.0, save_file = ''):
    Tools.logger_message(30,'Detecting Matches...')

    fields = fields or [Report.multiplierzHeaders[k]
    ...
    if save_file:
        writer = Report.writer(save_file, columns=cols)
    ...
    Tools.logger_message(20,'Matches Detected')
...
def _detect_matches(file_names, fields, tol_field, tolerance=0.0):
    ...
    if all_mzd:
        ...
        for name in file_names:
            report = Report.reader(name)

            Tools.logger_message(10, name)
...
def filter_join(file_names, key_source_file, exclude=False,
    ...
    if not save_file_suffix:
        raise ValueError('Save_file_suffix cannot be empty string')
    ...
    if append:
        ...
        combo_report = Report.writer(combo_out_file, columns=newcols)
        ...
        for (filename,columns,filedata) in filtered_files:
            ...
            rep = Report.writer(save_file_suffix.join(os.path.splitext(filename)),
                                  columns=columns)
            for row in filedata:
                rep.write(row)
...
def _filter_join(file_names, key_source_file, exclude, save_file_suffix = '_filtered'):
    ...
    if all_mzd:
        ...
        source_report = Report.reader(key_source_file)

        col_list = []
        for name in file_names:
            rdr = Report.reader(name)
        ...
        try:
            for name in file_names:
                Tools.logger_message(20, name)
                this_rep = Report.reader(name)
